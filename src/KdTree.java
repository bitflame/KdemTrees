...
//        else if (h.right != null && cmp <= 0 && n.xCoord < h.right.maximumX) contains(h.right, n, p);
    ...
    public Iterable<Point2D> range(RectHV rect) {
        /* Does the rect contain the node, if so, I think he said you are done which means it contains all the
        points below it, but I do not see how this can be. Test it! He may have meant to add it to the list of the
        points and keep going. Next check to see if the line at each node intersects the rectangle, if so, you
        have to check both sides of that node for points, if not, you can just visit one side. repeat these steps
        until you hit null. */
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Here is how much time it took:"+ time);
    }
}
